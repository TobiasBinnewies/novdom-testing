...
exec(Command, Args, Cwd) ->
    ...
    CheckedCommand_ = case Command_ of
        "./" ++ _ -> Command_;
        "/" ++ _ -> Command_;
        _ -> os:find_executable(Command_)
    end,

    case CheckedCommand_ of
        false -> {error, {no_such_file_or_directory, "No command found named " ++ Command}};
        _ ->
            Name = {spawn_executable, CheckedCommand_},
